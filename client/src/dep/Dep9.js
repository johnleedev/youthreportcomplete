...
function Dep9(props) {
  ...
  useMemo(()=>{ return (
    ...
    axios.get(`/dep/9`).then((결과)=>{ //부서별 수정할 것
      console.log(결과.data)
      let copy = [...결과.data]
      set부서(copy)
    })
  ...
  // classNames
   let [show, setshow] = useState(`중1`) //부서별 수정할 것
  ...
  // 각y_이름
  let y_main = dmain.filter(e => e.dn === `dep9`) //부서별 수정할 것
  ...
  let [group_n, setgroup_n] = useState(`9-1`)
      ...
      <div className='dep_main_inner'>
        ...
        <div className='dep_main_presentCondition'>
          ...
          <div className='dep_main_title'>
            {/* 1) 타이틀 */}           {/* 부서별 수정할 것 */}
            <div className='dep_main_text'>중등부</div> 
            {/* 2) 버튼 */}
