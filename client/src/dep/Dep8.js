...
function Dep8(props) {
  ...
  useMemo(()=>{ return (
    ...
    axios.get(`/dep/8`).then((결과)=>{ //부서별 수정할 것
      console.log(결과.data)
      let copy = [...결과.data]
      set부서(copy)
    })
  ...
  // classNames
  let [show, setshow] = useState(`4학년`) //부서별 수정할 것
  ...
  // 각y_이름
  let y_main = dmain.filter(e => e.dn === `dep8`) //부서별 수정할 것
  ...
  let [group_n, setgroup_n] = useState(`8-1`)
